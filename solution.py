...
def ones_at_the_end_other_reverse(x):
    fin = [fin.append(i) if i==1 else fin.insert(0,i) for i in x]
    return fin
# %%
'''
### Final Position
...
def data_normalization(X):
    ...
    scaler.fit(XX)
    X_normorlize=scaler.transform(XX)
    scaler.transform(XX)
    scaler.scale_
    return np.array(X_normorlize), scaler

X =  [[1., -1., 2., 1], [2., 0., 0., 2], [0., 1., -1., 3]]
print(data_normalization(X))
_, scaler = data_normalization(X)
